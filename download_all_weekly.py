...
# Ensure the `downloads` table exists with the given schema
conn.execute("""
...
while current_date <= end_date:
    ...
    # Check if this week is already in the table
    existing_entry = conn.execute("""
        SELECT 1 FROM downloads 
        WHERE week_number = ? AND year = ?
        LIMIT 1
    """, [iso_week, iso_year]).fetchone()

    if existing_entry:
        print(f"Week {iso_year}-W{iso_week} already exists in the database. Skipping.")
    else:
